...
const ColorGame: React.FC = () => {
  ...
  const startGame = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Error starting color detection");
      setIsRunning(false);
      setGameStatus('idle');
      setTimeLeft(120);
    }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-4">🎨 Color Detection Game</h1>
      <p className="text-gray-600 mb-4">Can you identify different colors in real-time?</p>

      <div className="text-sm text-gray-500 mb-6">
        <p><strong>Age Range:</strong> 4–12</p>
      ...
      <div className="mt-6 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700">
        <p>Webcam will open for color detection. Press <strong>Q</strong> to close early.</p>
        <p className="mt-2">Try to show different colored objects to the camera.</p>
      </div>
    </div>
  );
};
